...
void vControlTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        else if(recQueue == *(seat->set_to_ctrl_queue))
        {
            ...
            /*Calculates the desired temp of the state set by the user */
            switch (recState) {
            case LOW:
                desiredTemp = 25;
                break;
            ...
            displayData->desiredTemp = desiredTemp; /*Update display data*/

        }
        if(xErrorFlag == TRUE)
        {
            sentState = OFF; /*if there is an error the heater should be turned off*/
            xPrevError = TRUE;
            xEventGroupSetBits(xEventGroup, seat->error_reporting_mask); /*wake up the warning task to warn the user*/
        }
        else if(xErrorFlag == FALSE)
        {
            ...
            {
                xPrevError = FALSE;
                xEventGroupSetBits(xEventGroup, seat->error_handling_mask); /*if the previous error was handled, wake the warning task to abort the warning*/
            }
            else
            {
...
void vHeatingTask(void *pvParameters)
{
    ...
    for (;;)
    {
        xQueueReceive(recQueue, &state, portMAX_DELAY); /*Receive the state from the control task*/
        Heater_setTemp(seat->led, state); /*Set the state of the heater*/
...
void vTemperatureReadingTask(void *pvParameters)
{
    ...
    for(;;)
    {
        /*Wake up every 50ms to read the temp sensor of it's seat and send it to the control of this seat*/
...
void vTemperatureSettingTask(void* pvParameters)
{
    ...
    for(;;)
    {
        /*waits for the interrupt to set the flag */
        ...
        {
            state = (state + 1) % 4; /*update the state of the seat*/
            xQueueSend(sendToCtrlQueue,&state,0); /*send the state to the control*/
            xQueueSend(sendToSetQueue,&state,0); /*Send the state to the state update task*/
        }
...
void vTempErrorReportingTask(void* pvParameters)
{
    ...
    for(;;)
    {
        /*waits to receive an error code from the control task*/
        xQueueReceive(xCtrlToError, &errorCode, portMAX_DELAY);
        timeStamp = GPTM_WTimer0Read(); /*get a time stamp*/
        /*identify the error code and write the appropriate msg*/
        switch (errorCode) {
...
/*        update the error timestamp for the current log */
...
void vTempErrorWarningTask(void* pvParameters)
{
    ...
    for(;;)
    {
/*        waits for the control task to set an event bit */
        xEventBits = xEventGroupWaitBits(xEventGroup, RaisErrorMask|ErrorHandledMask, pdTRUE, pdFALSE, portMAX_DELAY);
/*        check which event bit was raised and take the appropriate action*/
...
void vStateDisplayTask(void* pvParameters)
{
    ...
    for(;;)
    {
/*        displays the data of both seats on the uart every 500 ms using the display data structs */
