...
/*used for task execution time measurments*/
...
/*this macro is called when a task going into the running state*/
#define traceTASK_SWITCHED_IN()                                    \
do{                                                                \
    uint32 taskInTag = (uint32)(pxCurrentTCB->pxTaskTag);          \
    ullTasksInTime[taskInTag] = GPTM_WTimer0Read();                \
}while(0);
/*this macro is called when a task is getting out of the running state*/
...
/***********************************For Resource Lock***************************************/
#define configQUEUE_REGISTRY_SIZE 20 /*Queue registery to use the queue names*/
extern uint32 ullTaskBlockTime[8];
extern uint32 ullTaskUnBlockTime[8];
extern uint32 ullTotalTaskBlockTime[8];

/*this macro is called when a task is going to be blocked on a queue receive,
 * it takes a timestamp upon whenever a task is blocked on a receive on a queue and stores
  in an array using the queue name in the queue registery  */
#define traceBLOCKING_ON_QUEUE_RECEIVE(xQueue)  \
do{                                              \
    uint8 queueTag =(uint8) pcQueueGetName(xQueue);    \
    ullTaskBlockTime[queueTag] = GPTM_WTimer0Read(); \
}while(0);

/* this trace hook macro is called when a task enters gets unblocked on a queue receive */
#define traceQUEUE_RECEIVE(xQueue) \
do{                                 \
    uint8 queueTag =(uint8) pcQueueGetName(xQueue);    \
    ullTaskUnBlockTime[queueTag] = GPTM_WTimer0Read();  \
    ullTotalTaskBlockTime[queueTag] = ullTaskUnBlockTime[queueTag] - ullTaskBlockTime[queueTag]; \
}while(0);
/*******************************FREE RTOS RUN TIME *********************************/
